...
function Shipping() {
  ...
  const [current, setCurrent] = useState(
    `${import.meta.env.VITE_API_URL}/shipping`
  );

  // fetching the category

  const Shipping = async (keywords = "") => {
    setLoader(true);
    try {
      ...
      if (response.status === 200) {
        setPages(response.data.shipping.links);
        setData(response.data.shipping.data);
        setLoader(false);
      }
    } catch (error) {
      console.error(error);
  ...
  const handleSearch = (keywords) => {
    setCurrent(`${import.meta.env.VITE_API_URL}/shipping?keywords=${keywords}`);
  };
          ...
          <div className="container-fluid my-2">
            <div className="row mb-2">
              ...
              <div className="col-sm-6 text-right">
                <Link to="/admin/shipping/create" className="btn btn-primary">
                  New Shipping
                </Link>
              </div>
            </div>
                ...
                <table className="table table-hover text-nowrap">
                  ...
                  <tbody>
                    {loader ? (
                      ...
                      data.map((value) => (
                        <ShippingList key={value.id} data={value} />
                      ))
                    )}
                  </tbody>
                </table>
