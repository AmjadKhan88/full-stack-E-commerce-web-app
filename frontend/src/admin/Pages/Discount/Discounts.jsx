...
function Discounts() {
  ...
  const Discount = async (keywords = "") => {
    ...
    try {
      ...
      if (response.status === 200) {
        setPages(response.data.discounts.links);
        setData(response.data.discounts.data);
        setLoader(false);
      }
    } catch (error) {
      console.error(error);
  ...
  const handleSearch = (keywords) => {
    setCurrent(`${import.meta.env.VITE_API_URL}/discount?keywords=${keywords}`);
  };
          ...
          <div className="container-fluid my-2">
            <div className="row mb-2">
              ...
              <div className="col-sm-6 text-right">
                <Link to="/admin/discounts/create" className="btn btn-primary">
                  New discounts
                </Link>
              </div>
            </div>
                ...
                <table className="table table-hover text-nowrap">
                  ...
                  <tbody>
                    {loader ? (
                      ...
                      data.map((value) => (
                        <DiscountList key={value.id} data={value} />
                      ))
                    )}
                  </tbody>
                </table>
