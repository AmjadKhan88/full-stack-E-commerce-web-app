...
function UpdateBrands() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/brands/update/${id}`,
        formData,
        {
          headers: {
  ...
  const oldBrands = async () => {
    setReloding(true);
    const token = localStorage.getItem("userToken");
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/brands/single/${id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
      ...
      if (response.status === 200) {
        ...
        setValue("name", response.data.brands?.name);
        setValue("slug", response.data.brands?.slug);
        setValue("status", response.data.brands?.status);
        setReloding(false);
      }
    } catch (error) {
      console.log(error);
    ...
    <>
      <div className="content-wrapper">
				...
				<section className="content">
                    ...
                    {(reloading)? <div className="spinner-border" role="status">
